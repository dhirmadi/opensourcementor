...
1) Provide your public SSH key to the hackathon host (for the `team1` user). The host will add it to `~team1/.ssh/authorized_keys`.

2) (Optional) Upload your key yourself if instructed:
```bash
ssh-copy-id -p 30022 -i ~/.ssh/id_ed25519.pub team1@<MINICLOUD_HOST>
```

3) SSH into the environment (SSH/SCP port is 30022):
```bash
ssh -p 30022 -i ~/.ssh/id_ed25519 team1@<MINICLOUD_HOST>
```
...
```bash
scp -P 30022 ./localfile team1@<MINICLOUD_HOST>:/srv/containers/team1/
```
...
```bash
ssh -p 30022 -i ~/.ssh/id_ed25519 -L 18080:127.0.0.1:8080 team1@<MINICLOUD_HOST>
